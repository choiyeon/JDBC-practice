...
public class RunPreparedStatementDAO {
	...
	public void addEmp() {
		...
		if (inputData != null) {
			...
			// 사원번호는 숫자 4자, 10000 이하 : 유효성 검사
			if (tempData[0].length() > 4) {
				JOptionPane.showMessageDialog(null, "사원번호는 0~9999까지만 입력해주세요.");
				return;
			} // end if
	...
	public void removeEmp() {
		...
		try {
			int empno = parseInt(inputData);// 사원번호의 범위는 : 0 ~ 9999 사이

			// DBMS테이블의 delete변경을 수행
			PreparedStatementDAO psDAO = PreparedStatementDAO.getInstance();
			int cnt = psDAO.deleteEmp(empno);

			// cnt가 0이면 입력한 사원번호가 없는 번호일때
			String msg = inputData + "번 사원정보를 확인하세요.";

			// cnt가 1건 이상일때, 입력한 사원번호가 존재할 때
			if (cnt != 0) {
	...
	public void menu() {
		...
		do {
			inputMenu = JOptionPane
					.showInputDialog("PreparedStatement 메뉴 선택\n1. 사원정보 추가\n2. 사원정보 변경\n3. 사원정보 삭제 \n4. 전체 사원 검색 \n5. 특정 사원 검색 \n6. 종료");
